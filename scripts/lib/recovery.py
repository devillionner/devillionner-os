...
def verify(manifest):
    ...
    for entry in data["snapshots"]:
        ...
        if entry["snapshot"] != str(path.parent / name):
            raise ValueError("Snapshot outside its recovery bundle")
        verify_entry(entry)
    return data
...
def create(home, description):
    ...
    entries = []
    # Failure retains partial snapshots for inspection, without publication.
    for source in sources:
        target = bundle / ("root" if source["source"] == "/" else "home")
