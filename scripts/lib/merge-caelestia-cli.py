#!/usr/bin/env python3
"""Merge Blueprint-owned Caelestia CLI settings without replacing user config."""
...
def main(argv: list[str]) -> int:
    if len(argv) != 3:
        print(
            "usage: merge-caelestia-cli.py CANONICAL_CLI_JSON DESTINATION",
            file=sys.stderr,
        )
        return 2
    ...
    try:
        ...
        print(f"merge-caelestia-cli: {exc}", file=sys.stderr)
        return 1
    return 0
