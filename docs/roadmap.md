...
- [x] Keep Spotify on the managed `devos-spotify` launcher with `Super+M`, URI routing, Spotify Connect and 0.95 compositor opacity.
- [x] Retire the old Bloom palette watcher/custom pane geometry and define the curated **current Lucid + ivLyrics + clean UI/wave/Oneko** stack.
- [ ] Apply the curated Spotify stack on the physical host and visually/runtime-validate it before marking the experience complete.
...
- [x] Make the live desktop Caelestia-first: Caelestia logo on top bar/dashboard/lock screen, suppress Cachy-Update background/tray UX while retaining its manual launcher, keep CachyOS Kernel Manager available on demand, hide branding-only shortcuts, and remove pure CachyOS welcome/package-installer/wallpaper extras.
...
- [x] Curated Lucid + ivLyrics Spotify integration documented.
- [x] Fragments + Caelestia background/tray integration documented.
