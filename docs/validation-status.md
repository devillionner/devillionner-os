...
| Service manifests | ✅ | ⏳ | ⏳ | ⏳ | `check-services` now verifies that every selected system/user unit exists and is enabled. CI locks explicit service↔package wiring for Bluetooth, NetworkManager, SDDM, UFW, Gaming ananicy, libvirt and common user services; TV Cast remains daemon-free. Runtime evidence is still pending. |
...
1. Finish the small existing-host checks when the laptop is available: SDDM state, service-manifest state, Spotify pane border and Bibata cursor after relogin.
