...
| Caelestia merge policy + package patches | ✅ | ✅ | ⏳ | ⏳ | Managed settings and QML patches are source-validated. On the existing host, the Nexus Wi-Fi password flow was exercised successfully against a secured SSID containing backslashes and a trailing space after exact SSID/BSSID handling was fixed. |
| Network UI ownership | ✅ | ✅ | ⏳ | ⏳ | Existing host runs NetworkManager with Caelestia as the only Wi-Fi UI; `network-manager-applet` / `nm-connection-editor` and stale nm-applet autostart are absent, while `libappindicator` is retained for FluxCast tray support. |
| Caelestia-first branding | ✅ | ✅ desktop/runtime | ⏳ | ⏳ | Existing host uses `general.logo=caelestia`, so top-bar/dashboard/lock branding is Caelestia. Cachy-Update tray/timer is suppressed while its manual launcher remains available; CachyOS Kernel Manager also remains available on demand. Branding-only shortcuts are hidden, and Hello/Package Installer/Wallpapers were removed. CachyOS remains the real technical base. Boot/login branding is deliberately pending the clean-KVM/SDDM reboot gate. |
...
| Fragments torrent integration | ✅ | ✅ | ⏳ | ⏳ | Existing host on 2026-09-24: `check-fragments` 42 OK / 0 FAIL after migration from the initial `special:fragments` scratchpad model to a standalone localhost Transmission user service. Fragments now closes normally and appears only on demand; background transfer continuity, single-daemon ownership, tray registration, GTK theme watcher and the dynamic torrent progress/speed submenu were exercised. User torrent data is not part of the Blueprint. |
