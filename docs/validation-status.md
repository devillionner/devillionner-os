...
| Caelestia-first branding | ✅ | ✅ desktop/runtime | ⏳ | ⏳ | Existing host uses `general.logo=caelestia`, so top-bar/dashboard/lock branding is Caelestia. Cachy-Update tray/timer is suppressed but the normal manual updater launcher remains available; branding-only shortcuts are hidden, and Hello/Package Installer/Wallpapers were removed. CachyOS remains the real technical base. Boot/login branding is deliberately pending the clean-KVM/SDDM reboot gate. |
